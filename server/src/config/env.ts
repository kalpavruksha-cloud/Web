...
const normalizedProcessEnv = {
  ...
  CLIENT_URL: normalizeUrl(process.env.CLIENT_URL || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : "http://localhost:5173")),
...
export const env = {
  ...
  corsOrigins: [
    ...data.CORS_ALLOWED_ORIGINS.split(",").map((origin) => normalizeUrl(origin)).filter(Boolean),
    normalizeUrl(data.CLIENT_URL),
    process.env.VERCEL_URL ? normalizeUrl(`https://${process.env.VERCEL_URL}`) : undefined
